...
#ifndef C_STRING_HEADER
#define C_STRING_HEADER
//#error "APPS_UCU_ERROR: pragma is not enough. Please, add correct include guards"

#include <stdlib.h>
#include <stdint.h> // For size_t
#include <stddef.h> // For size_t
//#error "APPS_UCU_ERROR: some important includes are missing"
#include <stdio.h>
 ...
 * if given size > size of string than files next bytes with given chars
 * return:
 *      0  if OK
 *      NULL_PTR_ERR if str is NULL
 *      MEMORY_ALLOCATION_ERR if there was an error during memory allocation
 ...
 *      NULL_PTR_ERR if str is NULL
 */
size_t my_str_find_c(const my_str_t* str, char toFind, int from);
 ...
 */
size_t my_str_find_if(const my_str_t* str, int beg, int (*predicate)(int));
